<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 space-y-5">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" class="mt-1 block w-full rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-sm focus:border-navy-500 focus:ring-navy-500 transition">{{ old('description', $product->description) }}</textarea>
                ...
                <div>
                    ...
                    @if ($product->image_path)
                        <div class="mt-2 mb-3 flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                            <img src="{{ Storage::url($product->image_path) }}" alt="{{ $product->name }}" class="w-16 h-16 object-cover rounded-md border border-slate-200 dark:border-slate-700">
                            <div>
                                <p class="text-xs font-medium text-slate-700 dark:text-slate-300">Imagem atual cadastrada</p>
                                <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Selecione um novo arquivo abaixo se desejar substituir</p>
                            </div>
                        </div>
                    @endif
